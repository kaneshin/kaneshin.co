  ...
  for (let fileToConvert of filesToConvert) {
    ...
    fs.writeFileSync(
      ...
      ampify(contents, urlPath, $ => {
        ...
        <script type="application/json">{"vars": {"account": "' +
            GA_TRACKING_ID +
            '"},"triggers": {"trackPageview": {"on": "visible","request": "pageview"}}}</script>\
        </amp-analytics>',
        );
        ...
        let css;
        css = sass.renderSync({ file: "src/assets/styles/index.scss", outputStyle: "compressed" }).css.toString();
